...
/**
 * Plain-language recommendation derived from the actual result, so the report
 * closes with a conclusion rather than just numbers.
 */
export function buildRecommendation(
  score: { score: number | null; categories: { name: string; score: number | null }[]; nonCompliantCount: number; partialCount: number },
  findingCount: number,
) {
